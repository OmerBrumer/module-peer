...
resource "azurerm_virtual_network_peering" "peering_src" {
  name                         = format("peering-to-%s", var.vnet_dest_name)
  resource_group_name          = var.vnet_src_resource_group_name
...
resource "azurerm_virtual_network_peering" "peering_dest" {
  name                         = format("peering-to-%s", var.vnet_src_name)
  resource_group_name          = var.vnet_dest_resource_group_name
